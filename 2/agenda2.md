...
Ubuntu (Unix系OS)では、以下のコマンドをよく使うので、覚えておくとC演習あたりでうれしくなれます。
`$ ls`
> 今いるディレクトリ(≒フォルダー)の中にあるフォルダーやファイルの一覧を表示します。Listの略

`$ cd ディレクトリ`
> 今いるディレクトリを変更します。cd のみの場合はhomeディレクトリ(初期位置)に戻ります。Change Directoryの略

`$ cp コピー元 コピー先`
> ファイルをコピーします。Copyの略

`$ mv　移動元 移動先`
> ファイルを移動します。場所が同じだと、名前変更になります。Moveの略

`$ mkdir 名前`
> ディレクトリを作成します。Make Directoryの略

`$ rmdir 名前`
> ディレクトリを削除します。Remove Directoryの略

`$ rm 名前`
> ファイルを削除します。Removeの略

`$ clear`
> ターミナルをいったんきれいに(起動状態と同じに)します。これ単体で使用します。今までのものは、上にスクロールすると出ています。
